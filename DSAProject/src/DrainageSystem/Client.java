...
public class Client {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			while (true) {
				...
				// kal wala code, with modifications

				processed = new HashMap<String, Boolean>();

				int tflow = getFlowFromPath("S", "T", Integer.MAX_VALUE);
	...
	private static int getFlowFromPath(String v1name, String v2name, int mcap) {
		...
		for (String nbrname : nbrnames) {
			...
			if (!processed.containsKey(nbrname) && 
					...
					rgraph.get(v1name).get(nbrname) > 0
	...
	private static boolean createLevelGraphFromRGraph() {
		...
		LinkedList<DinicPair> queue = new LinkedList<Client.DinicPair>();

		HashMap<String, DinicPair> processed = new HashMap<String, Client.DinicPair>();
		...
		while (queue.size() > 0) {
			...
			for (String nbrname : nbrnames) {
				...
				if (!processed.containsKey(nbrname) &&
					...
					rgraph.get(rp.vname).get(nbrname) > 0) {
